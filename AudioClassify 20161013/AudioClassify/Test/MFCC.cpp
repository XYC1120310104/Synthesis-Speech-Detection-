...
//特征值由以下组成：MFCC:39维  短时能量及其一阶二阶差分3维  LEnFR 1维 和LSTER 1维 共44维 
...
//	uint32_t nFrames = (nSamples - FRAME_LENGTH) / FRAME_SHIFT;
...
//	//计算过零率和短时能量
//	ZCR(data, nSamples, nFrames, zcr);
//	AMP(Frame_data, amp);
//	MatrixXf frame_features = MatrixXf(MFCC_ORDER * 3 + 5, nFrames - 8);
...
//	features = MatrixXf(FEATURE_DIM, seg_n);
//	RowVectorXf tempFREn = Frame_data.cwiseAbs().colwise().sum() / FRAME_LENGTH;
//	for (int i = 0; i < seg_n; i++)
//	{
//		uint32_t idex = 0;
//
//		features.block(0, i, MFCC_ORDER * 3 + 5, 1) = frame_features.block(0, i * SEGMENT_SHIFT, MFCC_ORDER * 3 + 5, MODEL_SEGMENT).rowwise().mean();
//		RowVectorXf tempFR = tempFREn.segment(i*SEGMENT_SHIFT, MODEL_SEGMENT);
//		float avgEn = tempFR.mean();
//		for (int j = 0; j < MODEL_SEGMENT; j++)
//		if (tempFR(j) - avgEn*0.3 < 0)
//			idex++;
//		features(3 * MFCC_ORDER + 5, i) = (float)idex / ((seg_n - 1)*SEGMENT_SHIFT + MODEL_SEGMENT);// LEnFR = sum(tempFREn < avgEn * 0.3)/FramePerRead;
//		//计算avSTE和LSTER
//		VectorXf temp = features.block(0, i, MFCC_ORDER * 3, 1);
//		MatrixXf avSTE = MatrixXf::Ones(MFCC_ORDER * 3, MODEL_SEGMENT);
//		for (int j = 0; j <MODEL_SEGMENT; j++)
//			avSTE.col(j) = avSTE.col(j).cwiseProduct(temp);
//		MatrixXf temp_LSTER = avSTE - frame_features.block(0, 4 + i*SEGMENT_SHIFT, MFCC_ORDER * 3, MODEL_SEGMENT);
//		for (int j = 0; j < MODEL_SEGMENT; j++)
//			temp_LSTER.col(j) = sign(temp_LSTER.col(j));
//		features.block(3 * MFCC_ORDER + 6, i, MFCC_ORDER * 3, 1) = (float)1 / (2 * MODEL_SEGMENT)*(temp_LSTER + MatrixXf::Constant(temp_LSTER.rows(), temp_LSTER.cols(), 1)).rowwise().sum();
...
//		features(6 * MFCC_ORDER + 6, i) = (float)HZCRR_sign.sum() / (2 * MODEL_SEGMENT);
//	}
//	//归一化
//	for (int i = 0; i < features.cols(); i++)
//	{
//		if ((features.col(i).maxCoeff() - features.col(i).minCoeff()) != 0)
//		{
//			VectorXf con_f = VectorXf::Constant(features.rows(), 1 / (features.col(i).maxCoeff() - features.col(i).minCoeff()));
//			VectorXf min_f = VectorXf::Constant(features.rows(), features.col(i).minCoeff());
//			features.col(i) = (features.col(i) - min_f).cwiseProduct(con_f);
//		}
//
//	}
//	if (sr != NULL)
...
int32_t CMFCC::CelMfcc(float* d, uint32_t length, MatrixXf& features)//读数组  d：wav数据点的数组 length:长度
{
	...
	for (int i = 0; i < seg_n; i++)
	{
		...
		//计算avSTE和LSTER
		//float temp_1 = cc.block(0, 4 + i*SEGMENT_SHIFT, 1, MODEL_SEGMENT).sum() / MODEL_SEGMENT;//cc
		...
		//cout << temp_LSTER_1 << endl;
		features(3 * MFCC_ORDER + 6, i) = (float)1 / (2 * MODEL_SEGMENT)*((sign(temp_LSTER_1)).sum() + 1);
		//cout << sign(temp_LSTER_1) + RowVectorXf::Constant(temp_LSTER_1.size(), 1) << endl;
		//features(3 * MFCC_ORDER + 6, i) = (float)temp_LSTER_1.sum() / (2 * MODEL_SEGMENT);
		//计算LENFR
